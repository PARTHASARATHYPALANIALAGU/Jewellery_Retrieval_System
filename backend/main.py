...
async def search_by_text(req: TextSearchRequest):
    try:
        ...
        # if len(raw_start) == 0:
        #     print("📉 Raw search returned 0 results. Refinement needed.")
        #     needs_refinement = True
        # else:
        #     # Check score of top result
        #     top_score = raw_start[0].get('score', -10.0)
        #     print(f"📊 Top Score (Raw): {top_score:.2f}")
        #     
        #     if top_score < -1.0: # Threshold for "probably bad match"
        #          print("📉 Low confidence. Refinement needed.")
        #          needs_refinement = True
